...
string getEncryptedText( string inpt )
{
    ...
    for( int i=0; i<extendedLen; i+=16,counter++ )
    {
        ...
        encryption( temp ) ;
        for( int j=0; j<16; j++ ) {
            encryptedTxt [i+j] = temp[j] ^encryptedTxt [i+j] ;
        }
    }
    for ( int i = 0; i < extendedLen; i++)
    {
...
string getDecryptedText( string Cipher_text )
{
    ...
    for ( int i = 0; i < extendedLen; i += 16 ,counter++)
    {
        ...
        for( int j=0; j<16; j++ )
        {
            decryptedTxt[i+j] = Cipher_text[i+j]^temp[j] ;
        }
    }
    string decr;
...
void write_file(string encrypted){
    ...
    for(int i=0;i<extendedLen;i++)msg[i]=encrypted[i];
    FILE *fp;
