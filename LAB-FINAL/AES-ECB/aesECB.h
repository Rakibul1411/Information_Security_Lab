...
string ecb_encryption(string inpt);
...
void mixColumn(unsigned char *state)
{
    ...
    // s1'=  s1.T2 + s2.T3 + s3 + s0
    // s2'=  s2.T2 +s3.T3 + s1 + s0
    // s3'=  s0.T3 + s3.T2 + s2 + s1
    for (int i = 0; i < 16; i++)
        state[i] = temp[i];
}
...
string getDecryptedText(string Cipher_text)
{
    ...
    string decr;
    decr.resize(extendedLen); // +1 bad disi
    for (int i = 0; i < extendedLen; i++)
    {
...
{
    rotationOfWord(prevWord); // 1 byte left circular shift
