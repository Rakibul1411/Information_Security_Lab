...
unsigned long long int init_vector;
...
string cbc_encryption(string inpt);
...
void g_funct(unsigned char *prevWord, int index)
{
    rotationOfWord(prevWord);
    substitutionWord(prevWord);
    prevWord[0] ^= roundConstant[(index / 16) - 1];
...
void expansionKey(unsigned char *originalKey)
{
    ...
    for (index = 0; index <= 15; index++)
        ExpandedKey[index] = originalKey[index];
    ...
    while (index < 176)
    {
        ...
        for (int i = 0; i < 4; i++) {
            prevWord[i] = ExpandedKey[index + i - 4];
        }

        if ((index % 16) == 0)
        {
            g_funct(prevWord, index);
        }

        for (int k = 0; k < 4; k++) {
            ExpandedKey[index + k] = prevWord[k] ^ ExpandedKey[index - 16 + k];
        }
...
string cbc_encryption(string inpt)
{
    ...
    fixedstream << bitset<16>(init_vector);
...
string getDecryptedText(string Cipher_text)
{
    unsigned char decryptedTxt[extendedLen];
    unsigned long long int init_vector1 = init_vector;

    stringstream fixedstream;
    fixedstream << bitset<16>(init_vector1);
    ...
    for (int i = 0; i < extendedLen; i += 16)
    {
        ...
        for (int j = 0; j < 16; j++)
        {
            temp[j] = Cipher_text[i + j];
        }
        decryption(temp);
        for (int j = 0; j < 16; j++)
        {
            decryptedTxt[i + j] = temp[j] ^ str[j];
            str[j] = Cipher_text[i + j];
        }
    }
    string decr;
    decr.resize(extendedLen);
    for (int i = 0; i < extendedLen; i++)
    {
